...
This app runs as a Node service on `127.0.0.1:3001`. Apache handles the public hostname and forwards requests to it. Ollama remains local on `127.0.0.1:11434`.
...
The app uses `OLLAMA_MODEL=dq-assistant:latest` by default. Override it in `/etc/systemd/system/dq-ai.service` if another installed model should be used. Browser conversation history stays in each user's local browser. The demo login password is stored in the frontend source and is not a multi-user authentication system; add real server-side authentication before exposing this publicly.
